...
export class Xpmlite {
  ...
  constructor(authInstance: AuthService, apiInstance: ApiClient, mediaInstance: Media, componentInstance: ComponentService) {
  ...
  private async getFieldType(fieldName: string, valueElement: HTMLElement, schemid: string): Promise<void> {
    ...
    if (rawContent !== "" && parentComponent && parentFieldName) {
      ...
      storageData[componentId][dataFieldName] = rawContent;
    ...
    let fieldDef = response.Fields[fieldName];

    if (!fieldDef) {
      for (const key in response.Fields) {
        if (response.Fields[key].EmbeddedFields?.[fieldName]) {
          fieldDef = response.Fields[key].EmbeddedFields[fieldName];
          break;
        }
      }
    }
    ...
    if (fieldDef) {
      ...
      if (fieldType === "XhtmlFieldDefinition") {
        this.richTextEditor(fieldName, valueElement);
      } else if (fieldType === "MultiLineTextFieldDefinition") {
        this.plainTextAreaEditor(fieldName, valueElement);
      } else if (fieldType === "SingleLineTextFieldDefinition") {
  ...
  private plainTextAreaEditor(fieldName: string, valueElement: HTMLElement): void {
    const targetId = `xpm-textarea-${Date.now()}`;
    const plainText = valueElement.textContent || "";

    valueElement.innerHTML = `<textarea name="${fieldName}" id="${targetId}" class="xpm-form-control" style="height:120px"></textarea>` + this.editorHandlers();
  ...
  private richTextEditor(fieldName: string, valueElement: HTMLElement): void {
    const targetId = `xpm-textarea-${Date.now()}`;
    const initialHtml = valueElement.innerHTML;
    valueElement.innerHTML = `<textarea name="${fieldName}" id="${targetId}" class="xpm-form-control" style="height:200px"></textarea>` + this.editorHandlers();
    const textarea = valueElement.querySelector(`#${targetId}`) as HTMLTextAreaElement;
    if (textarea) {
   ...
   /*  plugins: [
      'nonbreaking', 'anchor', 'autolink', 'charmap', 'code', 'codesample', 
      'image', 'link', 'searchreplace', 'table', 'visualblocks', 'wordcount'
    ],
    toolbar: 'undo redo | blocks | bold italic underline | link image table | code | removeformat', */
