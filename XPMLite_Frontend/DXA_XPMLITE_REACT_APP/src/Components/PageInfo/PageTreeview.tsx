...
import { setComponentPresentation, setExpandKeys, setSelectedKeys, updatePageInfodata } from "../../store/slices/pageInfoSlice";
...
const PageTreeview = () => {
    ...
    const renderTreeIcon = (props: any) => {
        const iconType = props.iconType || (props.isLeaf ? 'component' : props.children ? 'region' : 'page');
    ...
    const renderTreeTitle = (nodeData: any) => {
        if (nodeData.isLeaf) {
            return <ShowElipsis title={nodeData.title} />;
        }
                ...
                <div className="ant-tree-group">
                    {expandkeys.length !== 0 && componentPresentation.length !== 0 ? (
                        <DirectoryTree
                            ...
                            onSelect={getRegion}
                            treeData={componentPresentation as any}
                        />
                    ) : (
